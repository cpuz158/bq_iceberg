...
| **[`bq_iceberg_bigquery_dml.ipynb`](./bq_iceberg_bigquery_dml.ipynb)** | **BQ DML Test Notebook** | BigLake REST Catalog 기반 Apache Iceberg 테이블에 `'gcp.biglake.bigquery-dml.enabled' = 'true'` 옵션을 적용하여 BigQuery DML(INSERT, UPDATE, DELETE) 동작을 검증하는 독립 전용 노트북 |
...
| **[`bq_iceberg_spark_sql_dml.ipynb`](./bq_iceberg_spark_sql_dml.ipynb)** | **Spark SQL DML Maintenance Notebook** | PySpark & Spark SQL 기반 BigLake REST Catalog DML Iceberg 테이블 조회 및 Compaction(`rewrite_data_files`), Snapshot Expire(`expire_snapshots`) 유지보수 프로시저 실행 검증 노트북 |
...
| **[`pyproject.toml`](./pyproject.toml)** / **[`requirements.txt`](./requirements.txt)** | **Dependencies** | PySpark, PyIceberg 0.11.0+, DuckDB 1.5.4, BigQuery SDK 패키지 호환성 정의서 |
...
## 🛠️ BigQuery DML 지원 및 Spark SQL 유지보수 (`bq_iceberg_bigquery_dml.ipynb` & `bq_iceberg_spark_sql_dml.ipynb`)

BigQuery 엔진 및 Spark SQL 엔진에서 BigLake REST Catalog로 연동된 Apache Iceberg 테이블에 대한 direct DML(INSERT, UPDATE, DELETE) 및 Maintenance(Compaction, Expire Snapshots) 작업을 수행하기 위한 전용 테스트 노트북 시리즈입니다:

1. **BigQuery Direct DML 연동 (`bq_iceberg_bigquery_dml.ipynb`)**:
   - PyIceberg `DayTransform()` 파티셔닝 변환 및 `"gcp.biglake.bigquery-dml.enabled": "true"` 설정 적용
   - BigQuery SQL DML(INSERT, UPDATE, DELETE) 검증 (`f"{PROJECT_ID}-bq-iceberg-dml-bucket"` 기준)

2. **Spark SQL Iceberg Table Maintenance (`bq_iceberg_spark_sql_dml.ipynb`)**:
   - `BUCKET_NAME = os.getenv("GCS_BUCKET_NAME") or f"{PROJECT_ID}-bq-iceberg-dml-bucket"` 기반 실행
   - **Data Files Compaction (`rewrite_data_files`)**: DML 작업 후 생성된 소형 파키 파일(Small Files) 및 Delete Delta 파일들을 병합하여 Read Performance 극대화
     ```sql
     CALL lakehouse_catalog.system.rewrite_data_files(
         table => 'lakehouse_catalog.default.external_dml_weblog',
         options => map('rewrite-all', 'true')
     )
     ```
   - **Legacy Snapshots Expire (`expire_snapshots`)**: 지정 타임스탬프 이전의 레거시 스냅샷 및 고아 파일(Orphan files)을 정리하여 GCS 용량 최적화
     ```sql
     CALL lakehouse_catalog.system.expire_snapshots(
         table => 'lakehouse_catalog.default.external_dml_weblog',
         older_than => TIMESTAMP '<NOW_TIMESTAMP>',
         retain_last => 1
     )
     ```
